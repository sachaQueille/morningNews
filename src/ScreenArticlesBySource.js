  ...
  const news = articleList.map(function (article, i) {
    return (
      <Card key={article.title}
        ...
        cover={<img alt="example" src={article.urlToImage} style={{height: 200}} />}
        actions={[
          <Icon onClick={() => showModal(article.title, article.description)} type="read" key="ellipsis2" />,
          <Icon type="like" key="ellipsis" />,
        ]}
      >
        <Meta style={{height: 200}} title={article.title} description={article.description} />
